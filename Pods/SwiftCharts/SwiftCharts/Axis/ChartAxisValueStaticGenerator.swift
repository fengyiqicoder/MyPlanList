...
//          print("首个点 \(axisPicker(first).scalar) \(axisPicker(last).scalar) \(maxSegmentCount)")
          let firstDouble:Double = maxSegmentCount == 40 ? 0.0 : axisPicker(first).scalar
            return generateAxisValuesWithChartPoints(firstDouble, last: axisPicker(last).scalar, minSegmentCount: minSegmentCount, maxSegmentCount: maxSegmentCount, multiple: multiple, axisValueGenerator: axisValueGenerator, addPaddingSegmentIfEdge: addPaddingSegmentIfEdge)
